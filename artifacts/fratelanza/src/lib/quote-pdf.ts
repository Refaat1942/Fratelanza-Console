...
<style>
  ...
  * { box-sizing: border-box; }
  body {
    ...
    margin: 0; padding: 16px 20px; color: ${NAVY}; background: #fff;
    font-size: 16px; line-height: 1.45;
  }
  .header { position: relative; min-height: 72px; margin-bottom: 8px; }
  .logo {
    position: absolute; top: 0;
    ${isArabic ? "left: 0;" : "right: 0;"}
    width: 45mm; max-height: 18mm; object-fit: contain;
  }
  .company {
    font-size: 32px; font-weight: 700; color: ${NAVY}; margin: 0 0 4px 0;
    text-align: ${isArabic ? "right" : "left"};
    ${isArabic ? "padding-left: 50mm;" : "padding-right: 50mm;"}
  }
  .tax-reg {
    font-size: 12px; color: ${MUTED}; margin: 0 0 12px 0;
    text-align: ${isArabic ? "right" : "left"};
    ${isArabic ? "padding-left: 50mm;" : "padding-right: 50mm;"}
  }
  .divider { border: none; border-top: 2px solid ${ACCENT}; margin: 0 0 16px 0; }
  .doc-title {
    font-size: ${isArabic ? "18px" : "16px"}; font-style: ${isArabic ? "normal" : "italic"};
    color: ${MUTED}; margin: 0 0 12px 0; text-align: ${isArabic ? "right" : "left"};
  }
  .meta { font-size: 14px; margin: 0 0 6px 0; text-align: ${isArabic ? "right" : "left"}; }
  .meta strong { font-weight: 700; color: #000; }
  table.items {
    width: 100%; border-collapse: separate; border-spacing: 0;
    margin: 20px 0 8px 0; table-layout: fixed;
    border: 1px solid #d0d0d0; border-radius: 10px; overflow: hidden;
  }
  table.items th, table.items td {
    border: none; border-bottom: 1px solid #d0d0d0;
    padding: 10px 12px; vertical-align: middle; word-wrap: break-word;
  }
  table.items tbody tr:last-child td { border-bottom: none; }
  table.items th { background: ${ACCENT}; color: #fff; font-size: 16px; font-weight: 700; text-align: center; }
  table.items td.desc-col, table.items th.desc-col { text-align: ${isArabic ? "right" : "left"}; width: 100%; }
  .grand-total {
    display: flex; justify-content: ${isArabic ? "flex-start" : "flex-end"};
    align-items: center; gap: 16px; margin: 24px 0 8px 0;
    padding: 14px 18px; background: #f0f8ff; border: 2px solid ${ACCENT}; border-radius: 10px;
  }
  .grand-total-label { font-size: ${isArabic ? "18px" : "16px"}; font-weight: 700; color: ${NAVY}; }
  .grand-total-value { font-size: ${isArabic ? "22px" : "20px"}; font-weight: 700; color: ${ACCENT}; white-space: nowrap; }
  .section { margin: 14px 0 0 0; text-align: ${isArabic ? "right" : "left"}; }
  .section h3 { margin: 0 0 6px 0; font-size: ${isArabic ? "16px" : "14px"}; font-weight: 700; }
  .section.payment h3 { color: ${ACCENT}; }
  .section.milestones h3 { color: #28a745; }
  .section.outline h3 { color: #ffc107; }
  .section p { margin: 0; font-size: ${isArabic ? "14px" : "12px"}; color: #000; line-height: 1.65; }
  .outline-page { margin-top: 10px; padding-top: 8px; border-top: 1px dashed #ddd; }
  .outline-page-label { font-size: 11px; color: ${MUTED}; margin-bottom: 4px; font-weight: 600; }
  .footer { margin-top: 28px; font-size: 12px; font-style: italic; color: ${MUTED}; text-align: ${isArabic ? "right" : "left"}; }
...
<body>
  ...
  <table class="items">
    <thead><tr><th class="desc-col">${esc(labels.descHeader)}</th></tr></thead>
...
export function exportQuote(q: QuoteForPdf, opts: QuotePdfOptions = {}, format: QuoteExportFormat = "pdf") {
  const html = buildQuoteHtml(q, opts, format === "pdf");
  ...
  if (format === "word") {
    ...
    a.href = url;
    a.download = `Quote-${q.clientName.replace(/\s+/g, "-")}.doc`;
    a.click();
