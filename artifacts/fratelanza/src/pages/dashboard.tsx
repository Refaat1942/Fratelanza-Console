...
function Kpi({ label, value, icon: Icon, color, delay }: { label: string; value: number; icon: React.ComponentType<{ className?: string }>; color: string; delay: number }) {
      ...
      <Card className="bg-card/60 backdrop-blur border-border/60 hover:border-primary/40 transition-colors overflow-hidden relative">
        ...
        <CardContent>
          <div className="text-2xl font-bold text-foreground">
            {isPrivate ? <span>***</span> : (
              <span>EGP <AnimatedNumber value={value} format={(n) => n.toLocaleString(undefined, { maximumFractionDigits: 0 })} /></span>
            )}
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <Kpi label={t('dashboard.netProfit')} value={summary?.totalNetProfit ?? 0} icon={Activity} color="bg-primary/10 text-primary" delay={0.15} />
        <Kpi label={t('dashboard.totalExpenses')} value={summary?.totalExpenses ?? 0} icon={ReceiptText} color="bg-red-500/10 text-red-400" delay={0.2} />
      </div>
